...
## Pre-rebuild Phase 5 deploy session (ported forward 2026-05-20)

> These four entries were authored on `main` before the ReBuild kickoff
> (`29b65b8`) but never made it into the rebuild branch. They cover the
> first GCP deploy attempt and the demo blocker that triggered the rebuild
> conversation. Kept verbatim from `main:LEARNINGS.md` §L9.5–L9.8 for
> faithfulness — formatting is slightly different from the LR* entries
> above.
...
### L-S4b.2 — Three latent defects "hidden" by S4b.1 being shipped without the scrape ever running
**What bit us:** S4b.1 commit message claimed "structural PDF extractor emits StructuredContent tree" and the slice was marked Done. When S4b.2 attempted the actual real-RBI scrape — the slice's stated bar — three latent defects surfaced in the *first* execution, none of them caught by S4b.1's unit tests:
  (a) `scraper/tasks.py:304`'s `INSERT INTO circular_documents` omits the `structured_content` column entirely. The extractor (S4b.1) emits the tree; the persistence wiring was missing. Every scrape would have produced `structured_content IS NULL` rows, defeating the entire F5 closure claim.
  (b) `scraper/crawler/rbi_crawler.py:152`'s URL filter accepts every `rbi.org.in` anchor with text. Listing pages like `NotificationUser.aspx` include the global site nav (`Depts.htm`, `Regionaloffices.aspx`, `PastGovernors.htm`, ...) in their header/footer, so the SCR-1 PDF-magic-bytes guard caught ~85% of links as "not a PDF" while quietly burning a fetch on each. The slice would have produced ≤20% of attempted URLs as real circulars; the GCP scrape stats showing "108 documents indexed" likely masked a 5× junk-fetch overhead.
  (c) `docker-compose.yml`'s scraper command was `celery -A celery_app ...` but the Dockerfile's `COPY . /app/scraper/` requires `scraper.celery_app`. The local docker compose stack's scraper service had **never** started successfully (it restart-loops on import error). The GCP Cloud Run Job worked because it uses its own CMD; local compose's scraper definition had been latently broken since the SCR-1 fix.

**Root cause:** S4b.1's "ship the extractor, defer the scrape" stage gate let the persistence/crawler/compose paths regress unnoticed. The unit tests covered the extractor in isolation (PDF bytes → tree). They did not cover the *path from* "extractor returns a tree" *to* "structured_content JSONB lands in PG." Stage-gating "extractor done" without an end-to-end smoke test against any PDF (real or fixture) created the gap. Compounded by: the GCP Cloud Run Job's success masked the local-compose breakage, and the SCR-1 PDF-magic-byte fix's graceful-failure semantics masked the URL-filter overpicking.

**Fix:**
  (a) `scraper/tasks.py` — INSERT now includes `structured_content` (JSONB cast, with `json.dumps(extracted.structured_content)` when present, NULL otherwise).
  (b) `scraper/crawler/rbi_crawler.py` — URL filter restricted to `.pdf` extension OR `NotificationUser.aspx?Id=NNNN` OR `BS_ViewMasdirections.aspx?Id=NNNN`. Drops global-nav HTML anchors at the source instead of relying on the magic-byte guard.
  (c) `docker-compose.yml` — celery `-A scraper.celery_app`, `PYTHONPATH=/app`. Both `scraper` and `celery-beat` services were affected.

Net effect: local `python -m scraper.run_oneshot priority` produced **150 circulars with structured_content** in ~10 minutes against rbi.org.in (vs 0 before).

**How to prevent:**
1. **A stage gate that ships only the extractor is invalid.** A "structural PDF extractor" slice must include at least one persistence path through `scraper/tasks.py` + one PDF round-trip (fixture or fetched) ending in a row with `structured_content IS NOT NULL`. The integration-test suite should grow a `test_scraper_persistence.py` that runs the extractor → INSERT → SELECT cycle against pgvector.
2. **A scraper change that doesn't run against `docker compose up scraper` is invalid.** GCP Cloud Run Job success ≠ local-compose success when their CMDs diverge. The latent compose-path break would have been caught by any single attempted local run.
3. **URL filtering is the scraper's contract.** The "graceful PDF failure" guard (SCR-1) is the safety net, not the policy. If the magic-byte check is doing more than ~5% of the rejection work, the URL filter is too permissive — file a bug, don't celebrate the safety net.
4. **Answer-cache hits must persist a per-user `Question` row.** The pre-S4b.2 cache code returned the original cached `id` (belonging to whoever asked first) to every subsequent user — invisible until the corpus was big enough to actually get cache hits. Save → /saved → /history broke cross-user because user B's POST `/saved` tried to bookmark user A's question. Fix: on cache hit, INSERT a new Question for the current user (no credit deduction), return that new id. Same gate as (1) — without a populated corpus, the bug couldn't surface.
5. **SSE branches must enumerate all early-return paths.** Cache hit + no-chunks consult-expert + happy-path retrieval are three distinct branches; each needs its own `if "text/event-stream" in accept` check. Missing one (cache hit, fixed in this slice) means the v2 Ask page hangs forever on a warm cache.
