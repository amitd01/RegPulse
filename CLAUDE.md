...
| S4b | 2b | Scraper end of slice 2: structural PDF extractor; dual-output chunker; real RBI scrape of ≥20 circulars; Playwright `library.spec.ts` against real data; integration test for `/circulars/{id}` | Done — 150 RBI circulars ingested with `structured_content` populated by `extract_pdfplumber_full` → S4b.1 extractor + dual-output persistence wiring in `scraper/tasks.py`; crawler URL filter tightened (rule: only `.pdf` or `?Id=NNNN` on Notification/MasterDirection endpoints — was picking up site-nav HTML pages); `docker-compose.yml` celery module path fixed (`scraper.celery_app`, PYTHONPATH=/app); answer-cache hit now persists a per-user `Question` row + streams SSE so Save/History works after cache warm; Playwright `library.spec.ts` (4 cases) + integration `test_circulars.py` (3 cases) green; 14/14 Playwright, 155/155 unit |
